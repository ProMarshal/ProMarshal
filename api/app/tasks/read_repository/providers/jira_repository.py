"""Jira-backed task read repository over Brain task snapshots."""
...
class JiraTaskReadRepository:
    """Jira read repository using project-scoped Brain collection."""
    ...
    def _to_task_response_dto(cls, payload: Dict[str, Any]) -> TaskResponseDTO:
        return TaskResponseDTO(
            ...
            status_raw=(str(payload.get("status_raw") or "").strip() or None),
            status_display=(
                str(payload.get("status_display") or payload.get("status_raw") or "").strip()
                or None
            ),
    ...
    def _build_list_query(cls, filters: ListTasksFilters) -> Dict[str, Any]:
        ...
        # 1) If caller/user Jira ID is available, match by ID fields only.
        # 2) Use email/name matching only when user Jira ID is absent.
        if assignee_user_id:
        ...
        provider_types = [
            str(item or "").strip()
            for item in (filters.provider_types or [])
            if str(item or "").strip()
        ]
        if provider_types:
            type_or_clauses: List[Dict[str, Any]] = []
            for provider_type in provider_types:
                escaped = re.escape(provider_type)
                regex = {"$regex": f"^{escaped}$", "$options": "i"}
                type_or_clauses.append({"provider_type": regex})
                type_or_clauses.append({"issue_type": regex})
            and_clauses.append({"$or": type_or_clauses})
